...
При наличии свободного времени можно также изучить:
    ...
    * Прочитай приложение о вычисление задержек на дорожках платы DDR3.
...
## Приложение - Вычисление задержек на дорожках платы DDR3

Zynq позволяет сконфигурировать до 4 устройств памяти для топологии DDR3 4x8 fly-by. Плата ZedBoard настроена для топологии DDR3 2x16 fly-by. Описание топологии fly-by приведено в документе Micron TN-41-13. На рис. 1 и рис. 7 показана базовая топология цепи, используемая для команд/адресов/управления/тактовых импульсов.

Для двух микросхем DDR3 все DQS и DQ являются соединениями "точка-точка".  Значения для этих соединений можно легко получить из простого отчета о длине дорожки.  Однако из-за "fly-by" топологии CLK разводится последовательно.  На ZedBoard имеется только одна пара тактовых импульсов DDR3, которая сначала поступает на IC26, а затем на IC25.  Длина линии CLK в отчете будет являться общей длиной.  Получить длины CLK2 и CLK3 будет сложнее, для этого придется открыть схему.

Во-первых, давайте рассчитаем значения длин, которые мы легко можем получить из отчета длин дорожек (Trace Length Report).

1. Отчет длин дорожек для ZedBoard представлен в папке *Support_documents*. Откройте `ZedBoard_RevB_PCB_Trace_Length_Report.txt`.
